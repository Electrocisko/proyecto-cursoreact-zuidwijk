...
function CartItem({ item }) {
  ...
  const addOneUnitStorage = () => {
    if (controlDeStock > 0) {
      ...
      swal({
        text: "No se puede agregar mas items",
        buttons: false,
        timer: 1000,
      });
    }
  ...
  return (
    <div className="cart-container">
      ...
      <div className="container-cart-quantity">
        <button className="cart-button" onClick={() => removeUnitStorage()}>
          <i className="fa-solid fa-minus"></i>
        </button>
        <p className="producto__cart__quantity">{item.quantity} U</p>
        <button className="cart-button" onClick={() => addOneUnitStorage()}>
          <i className="fa-solid fa-plus"></i>
        </button>
      </div>
      <div className="container-cart-precio">
        <p className="producto__cart__precio">
          ${(item.price * item.quantity).toLocaleString()}{" "}
        </p>
      </div>
      <div className="container-trash">
        <button className="cart-button" onClick={() => removerPorId()}>
